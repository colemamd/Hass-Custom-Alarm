class BWAlarmControlPanelCard extends HTMLElement {
    constructor() {
        super();
    ...
    setConfig(config) {
        ...
        if (config.auto_enter) {
            if (!config.auto_enter.code_length || !config.auto_enter.arm_action) {
                throw new Error('Specify both code_length and arm action when using auto_enter.');
            }
    ...
    _updateCardContent(entity) {
        ...
        root.getElementById("state-icon").setAttribute("icon", this._icons[this._state] || 'mdi:shield-outline');
    ...
    _actionButtons() {
        const armVisible = (this._state === 'disarmed');
        return `
            <div id="arm-actions" clas="actions"> ${this._config.states.map(el => `${this._actionButtons(el)}`).join('')}
            </div>
            <div id="disarm-actions" class="actions"> ${this._actionButton('disarm')}
            </div>`;
    }

    _actionButton(state) {
        return `<paper-button noink raised id="${state}"> ${this._label("ui.card.alarm_control_panel." + state)}</paper-button>`;
    }
    ...
    _label(label, default_label=undefined) {
        // Just show "raw" label; useful when you want to see underlying
        // const so you can define your own label.
...
customElements.define('alarm_control_panel-card', BWAlarmControlPanelCard);